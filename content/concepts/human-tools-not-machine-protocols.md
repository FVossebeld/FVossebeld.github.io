...
There is also a historical reason the machine protocols stuck. Early tool-using models were worse, so rigid JSON was how you kept them usable at all. Structured output gave the runtime something dependable even when the model itself was shaky. That was a good hack. I just don't see why the hack should remain the model-facing action layer once the model is strong enough to handle something richer.
