...
The safer default is a wider boundary with an [[approval-gate]] on the mutations that carry real risk. Let the agent read and write inside its scope. Put the gate on the actions that can hurt: a bulk data change, a customer-facing message, anything you can't [[rollback]]. Use a [[dry-run]] to show the proposed change before the gate opens. Then relax the gate over time as the [[audit-trail]] shows it makes good decisions. You don't need to strangle the scope to stay safe. You need to instrument the space inside it.
