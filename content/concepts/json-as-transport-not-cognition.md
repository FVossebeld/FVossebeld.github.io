...
JSON tool calling was the obvious next step after chat. We reached for it because it was already everywhere and models could already produce it. If you used the early function-calling stacks, the failure mode was obvious: wrong keys, missing fields, half-formed calls. A schema cleaned that up enough for a runtime to validate and execute it. Later structured-output systems tightened that further. OpenAI's [Structured Outputs write-up](https://openai.com/index/introducing-structured-outputs-in-the-api/) is explicit about the mechanism: constrained decoding against the supplied schema, so only schema-valid continuations stay available during generation. That is a real capability. It is still a reliability hack, not some deep statement about how agents should act. Then the hack quietly became the default mental model for what "an agent doing something" _is_: emit an object, get an object back.
