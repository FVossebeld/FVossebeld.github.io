...
JSON tool calling was the obvious next step after chat. We reached for it because it was already everywhere and models could already produce it. More than that, older models needed the rails. A schema let you force a flaky model into a shape a runtime could validate and execute. That was a useful reliability hack, not some deep statement about how agents should act. Then the hack quietly became the default mental model for what "an agent doing something" *is*: emit an object, get an object back.
