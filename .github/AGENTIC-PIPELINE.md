...
| [`.github/anti-slop-field-guide.md`](./anti-slop-field-guide.md) | The full catalogue behind the charter: sixteen AI-tell families (A to P), each with a real example, a citation, and the fix. |
...
version, and `wiki-librarian` (or you) acts on it. Both keep the human as the merge gate, the workflow only opens an issue, the skills only open PRs.
